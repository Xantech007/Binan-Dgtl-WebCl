...
$pdo->prepare("UPDATE user_vip SET status=0 WHERE end_time <= NOW()")->execute();
...
/* GET ALL USER VIP HISTORY (for one-time check) */
$stmt = $pdo->prepare("SELECT vip_id FROM user_vip WHERE user_id=?");
$stmt->execute([$user_id]);
$ever_activated = array_column($stmt->fetchAll(PDO::FETCH_ASSOC), 'vip_id');

/* GET CURRENT ACTIVE VIPS */
$stmt = $pdo->prepare("SELECT * FROM user_vip WHERE user_id=? AND status=1");
$stmt->execute([$user_id]);
...
if(isset($_POST['activate_vip'])){
    ...
    // Check if already activated before
    if(in_array($vip_id, $ever_activated)){
        $_SESSION['vip_error'] = "You have already unlocked VIP $vip_id before. Each VIP level can only be activated once.";
    ...
    /* SAVE VIP */
    $pdo->prepare("INSERT INTO user_vip (user_id, vip_id, start_time, end_time, status)
                   VALUES (?,?,?,?,1)")
         ->execute([$user_id, $vip_id, $start, $end]);

    /* UPDATE USER VIP LEVEL (ONLY IF HIGHER) */
    if($vip_id > $current_vip_level){
        $pdo->prepare("UPDATE users SET vip_level=? WHERE id=?")
...
<div class="vip-container">
    <?php if(isset($_SESSION['vip_msg'])): ?>
        <div class="vip-success">
            <?php echo $_SESSION['vip_msg']; unset($_SESSION['vip_msg']); ?>
        </div>
    <?php endif; ?>

    <?php if(isset($_SESSION['vip_error'])): ?>
        <div class="vip-error">
            <?php echo $_SESSION['vip_error']; unset($_SESSION['vip_error']); ?>
        </div>
    <?php endif; ?>
    ...
    <?php while($vip = $vipQuery->fetch(PDO::FETCH_ASSOC)): 
        ...
        $alreadyUnlocked = in_array($vip_id, $ever_activated);
    ?>
        <div class="vip-card">
            <div class="vip-label">VIP<?php echo $vip_id; ?></div>
            ...
            <div class="vip-row">
                ...
                <div class="vip-details">
                    ...
                    <div class="value green"><?php echo number_format($vip['daily_profit'],2); ?></div>
            ...
            <div class="vip-action">
                ...
                <?php elseif($alreadyUnlocked): ?>
                    <button class="btn-disabled" disabled>
                        Already Unlocked
                    </button>
                <?php else: ?>
                    <button onclick="openPopup(<?php echo $vip_id; ?>)">
                        <?php echo number_format($vip['activation_fee'],2); ?> USDT<br>
                        Unlock now
                    </button>
                <?php endif; ?>
            </div>
...
<div class="vip-popup" id="vipPopup">
    <div class="popup-box">
        ...
        <form method="POST">
            ...
            <button type="submit" name="activate_vip" class="confirm-btn">Confirm</button>
