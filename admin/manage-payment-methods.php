...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    try{
        ...
        $paystack_link = trim($_POST['paystack_link'] ?? '');
        ...
        if($action === "add"){
            ...
            (
                name, wallet_address, qr_image, image, crypto, type, network,
                account_name, account_number, paystack_link, currency,
        ...
        if($action === "edit"){
            ...
            UPDATE payment_methods SET
                ...
                network=?, account_name=?, account_number=?, paystack_link=?,
                currency=?, conversion_rate=?, active_country=?, min_withdraw=?,
                status=?, withdrawal_fee=?
            WHERE id=?
            ");
            $stmt->execute($data);
...
<input type="url" name="paystack_link" placeholder="https://paystack.com/pay/xxxxx" style="width:100%;padding:0.8rem;">
</div>
</div>
...
<td style="padding:1rem;text-align:center"><?= htmlspecialchars($m['type']) ?></td>
...
<td style="padding:1rem;text-align:center"><?= $m['status'] ? 'Active' : 'Inactive' ?></td>
<td style="padding:1rem;text-align:center">
<button class="btn" style="margin-right:6px" onclick='openEditModal(<?= json_encode($m, JSON_HEX_APOS | JSON_HEX_QUOT) ?>)'>Edit</button>
...
<input type="text" name="network" id="edit_network" placeholder="Network / Bank" style="width:100%;padding:0.8rem;">
</div>
<div style="margin-bottom:1rem;">
<input type="text" name="account_name" id="edit_account_name" placeholder="Account Name" style="width:100%;padding:0.8rem;">
</div>
<div style="margin-bottom:1rem;">
...
<div id="editPaystackSection" style="display:none;">
<div style="margin-bottom:1rem;">
<label>Paystack Payment Link</label>
<input type="url" name="paystack_link" id="edit_paystack_link" style="width:100%;padding:0.8rem;">
</div>
</div>

<button class="btn" style="width:100%;padding:1rem;">Save Changes</button>
</form>
</div>
</div>
</main>
...
function openEditModal(m){
    ...
    document.getElementById("edit_paystack_link").value = m.paystack_link || "";
    toggleEditFields();
}
